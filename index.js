...
function getContext() {
    return (globalThis.SillyTavern || hostWindow.SillyTavern)?.getContext?.() || null;
}
...
function getSettings() {
    ...
    settings.injectionSocial = false;
    ...
    settings.socialAutoEnabled = false;
    ...
    // 当前改进版不提供通讯与悬浮球入口；保留旧字段仅用于读取旧数据。
    settings.injectionSocial = false;
    settings.socialAutoEnabled = false;
    settings.imageApiEnabled = false;
    settings.orbEnabled = false;
    settings.orbEdgeHide = false;
    settings.orbPosition = null;
...
function dismissPublicOpinionItem(kind, itemId) {
    ...
    if (!item) throw new Error('该舆情条目已不在当前列表中。');
...
function preemptLowPriorityTasksForCore({
    ...
    if (!hardAbort) {
        ...
        if (runtime.activeHistoryScan && !runtime.activeHistoryScan.signal.aborted) {
            runtime.historyProgress = {
                ...runtime.historyProgress,
                message: '新正文已经排队。我先把当前这批保存，再把执行权交给世界推演',
            };
            runtime.ui?.render();
        }
        return;
...
async function backgroundSimulation(prompt, {
    ...
    if (route.mode === 'tavern-profile') {
        ...
        if (!route.profile) {
            throw new Error(`酒馆已保存方案已不存在（ID: ${profileId}）。未自动切换到其他连接，请重新选择。`);
        }
        const service = context?.ConnectionManagerRequestService;
...
directorNotes: normalizeLingqiState(getStore().lingqi || emptyLingqiState()).notes
        ...
        setSyncStatus({
            phase: supersededByNewerReply ? 'pending' : 'success',
            message: supersededByNewerReply
                ? '这一轮推演已经安全存档。但当前正文已经更新，旧结果不会覆盖当前状态，接下来将处理最新一轮。'
                : '最新正文已完成推演',
            error: '',
            succeededAt: supersededByNewerReply ? '' : new Date().toISOString(),
            method: runtime.syncStatus.method,
    ...
    if (!coreAlreadyRunning && !autoTaskIsDue) {
        setSyncStatus({
            ...
            message: !settings.worldAutoEnabled
                ? `已有 ${pending.length} 轮正文待同步；自动推演为手动，本次发送不会自动启动推演`
    ...
    const report = {
        ...
        sillyTavern: {
            version: String(
                ...
                || hostDocument.querySelector?.('#version_display')?.textContent
                || '未识别',
            ).trim().slice(0, 120),
        },
        device: {
...
function deleteApiProfile(profileId) {
    ...
    saveSettings();
    runtime.ui?.render();
    toast(`“${existing.name}”已删除。`, 'success');
    return true;
}
...
async function testApiProfileConnection(profileId) {
    ...
    try {
        ...
        toast(`“${profile.name}”连接成功。`, 'success');
        return true;
    } finally {
        setBusy(false);
    }
...
async function pullApiProfileModels(profileId) {
    ...
    try {
        ...
        runtime.modelPullStatus = {
            phase: 'success',
            message: `找到 ${models.length} 个模型。还是可以手动填写哦`,
        };
        return models;
    } catch (error) {
...
function settingsFromApiDraft(payload = {}, { requireModel = true } = {}) {
    ...
    if (requireModel && !model) throw new Error('请先选择或填写模型。');
    return {
        ...base,
        apiMode: 'custom',
        customApiUrl: url,
...
async function testApiDraftConnection(payload = {}) {
    ...
    try {
        ...
        if (!String(reply || '').trim()) throw new Error('接口没有返回内容');
        toast(`${String(payload.label || '这个接口').slice(0, 80)}连接成功。`, 'success');
        return true;
    } finally {
        setBusy(false);
    }
...
async function pullApiDraftModels(payload = {}) {
    ...
    try {
        ...
        runtime.modelPullStatus = {
            phase: 'success',
            message: `找到 ${models.length} 个模型。不会改动默认接口`,
        };
        toast(`找到 ${models.length} 个个可用模型。`, 'success');
        return models;
    } catch (error) {
        runtime.modelPullStatus = { phase: 'error', message: describeError(error) };
        throw error;
...
async function pullCustomApiModels() {
    ...
    runtime.modelPullStatus = { phase: 'running', message: '正在获取模型列表。' };
    runtime.ui?.render();
    try {
        const context = getContext();
        ...
        runtime.modelPullStatus = {
            phase: 'success',
            message: `找到 ${models.length} 个模型。还是可以手动填写`,
        };
        toast(`找到 ${models.length} 个个可用模型。`, 'success');
        return models;
    } catch (error) {
    ...
    const payload = await runWithRetries(async attempt => {
        ...
        onRetry: ({ attempt, total, delayMs, rateLimited }) => {
            runtime.historyProgress.message = rateLimited
                ? `历史路线被限流。冷却 ${cooldownSeconds(delayMs)} 秒后只继续这一份记忆压缩`
                : `记忆压缩没保存，正在用更紧凑格式重试 ${attempt}/${total}`;
            runtime.ui?.render();
        },
        ...retryTaskOptions(
            'history',
    ...
    try {
        ...
        if (!quiet) {
            setSyncStatus({
                phase: 'success',
                message: manualPersonCatchUp
                    ? `${requestedPersonTarget.name} 的近况补齐。没有额外推进世界时间`
...
async function bootstrapWorldFromHistory() {
    ...
    const confirmed = globalThis.confirm?.(
        (checkpoint
            ? `  上次已经安全保存前 ${resumeCursor} 层，将从第 ${resumeCursor} 层继续，共剩约 ${Math.max(0, chatLength - resumeCursor)} 条消息。\n`
            : `  将从第 0 层开始回溯当前分支，共约 ${chatLength} 条消息。\n`)
    ...
    try {
        while (cursor < chatLength) {
            ...
            runtime.historyProgress = {
                ...
                total: chatLength,
                message: `正在回溯消息 ${batch.startMessageId}—${batch.endMessageId}：人物、暗流、事实和世界脉搏一起整理。`,
            };
            runtime.ui?.render();
                ...
                payload = await runWithRetries(async attempt => {
                    ...
                    onRetry: ({ attempt, total, delayMs, rateLimited }) => {
                        runtime.historyProgress.message = rateLimited
                            ...
                            : `这批世界基线没保存，正在用紧凑格式重试 ${attempt}/${total}`;
                        runtime.ui?.render();
                    },
                    ...retryTaskOptions(
                        'history',
        ...
        toast(
            `世界基线接入完成。人物 ${stagedState.people.length} · 世界事实 ${stagedState.worldFacts.length} · `
...
async function scanStoryMemoryHistory({
    ...
    if (cursor >= chatLength && !initialRollupPlan) {
        if (!automatic) toast('历史档案已经追到最新一层。', 'info');
        return true;
    }
    ...
    try {
        ...
        while (cursor < chatLength && completedBatches < batchLimit) {
            ...
            if (runtime.queuedSimulations.size > 0) {
                runtime.historyProgress.message = '当前记忆批次已经保存。世界推演在排队，先把路让出去';
                runtime.ui?.render();
                break;
            }
        }
...
async function observePerson(personId, { force = false } = {}) {
    ...
    try {
        ...
        if (stale) {
            const error = new Error('观测期间世界状态已经更新。旧观测结果未保存；重新观测将以最新状态为准。');
        ...
        if (isAbortError(error) || controller.signal.aborted) {
            setSyncStatus({
                phase: 'pending',
                message: '新正文已到达。旧人物观测先停掉，不会保存过期结果',
                error: '',
                method: runtime.syncStatus.method,
            });
            throw error;
...
async function generatePublicOpinionSnapshotInternal({ allowDefer = true, ensurePublicWorld = false, settlePublicImpact = true, force = false } = {}) {
    ...
    if (allowDefer && coreSimulationBusy()) {
        ...
        runtime.publicOpinionStatus = {
            phase: 'queued',
            message: '世界主线还在推演。舆情先排到后面，等核心状态追上就自动继续 ``',
            error: '',
        };
        runtime.ui?.render();
        return null;
    ...
    if (!plan.due) {
        ...
        runtime.publicOpinionStatus = {
            phase: 'success',
            message: '检查完成。公开来源没有新变化，世界时间也还没走到下一次舆情/新闻演化节点。',
            error: '',
        };
        runtime.ui?.render();
    ...
    try {
        ...
        if (stale) {
            ...
            runtime.publicOpinionStatus = {
                phase: 'pending',
                message: '世界刚刚又往前走了一步。当前新闻先留着，正在接最新一版 ``',
                error: '',
            };
            runtime.ui?.render();
        ...
        if (isAbortError(error) || controller.signal.aborted) {
            if (currentChatToken() === chatTokenAtStart) {
                ...
                runtime.publicOpinionStatus = {
                    phase: 'queued',
                    message: '检测到新正文。旧舆情任务已停止；待世界推演同步后再生成最新舆情。',
                    error: '',
                };
                runtime.ui?.render();
                scheduleDeferredPublicOpinion(260);
...
async function generatePublicOpinionSandbox() {
    ...
    runtime.publicOpinionSandboxStatus = {
        phase: 'running',
        message: '正在街上随便逛逛。看看今天有什么无关紧要的小热闹 ``',
        error: '',
    };
    runtime.ui?.render();
        ...
        try {
            ...
            if (isAbortError(error) || controller.signal.aborted) {
                if (currentChatToken() === chatTokenAtStart) {
                    runtime.publicOpinionSandboxStatus = {
                        phase: 'idle',
                        message: '本次自动舆情浏览已停止。需要时可再次手动触发。',
                        error: '',
                    };
                    runtime.ui?.render();
                }
...
function clearPublicOpinionSandbox() {
    ...
    saveStore(store);
    runtime.publicOpinionSandboxStatus = { phase: 'idle', message: '闲逛小报收起已到达。', error: '' };
    runtime.ui?.render();
    return true;
}
...
function finalizeLingqiTriage(response, actionResults = [], userText = '') {
    ...
    // 明确属于上游/API 的常见错误先去解决接口，不占维护者的插件反馈。
    ...
    if (triage.route === 'mama') {
        ...
        if (!triage.reason) {
            triage.reason = '现有帮助知识和实时状态都不能解释这个现象，需要维护者进一步检查插件行为。';
        }
    }
...
function lingqiActionConfirmation(action) {
    ...
    if (action.type === 'simulate_latest') {
        return {
            title: '推演最新正文？',
            detail: '执行一次世界推演；如果当前状态已经同步，则不会重复调用接口。',
            confirmLabel: '推演',
        };
    }
        ...
        try {
            if (LINGQI_CONFIRM_ACTION_TYPES.has(action.type) && !confirmed) {
                if (action.type === 'simulate_latest' && !latestNarrativeSyncSnapshot().needsSimulation) {
                    results.push('最新正文已经同步，无需重复调用接口。');
                    continue;
                }
                if (action.type === 'catch_up_person' && !findLingqiPersonTarget(action)) {
                    results.push('无法确认目标人物：可能存在重名或未找到匹配项。');
                    continue;
                }
                if (runtime.pendingLingqiAction) {
                    results.push('当前已有待确认操作，请先处理该操作。');
                    break;
                }
            ...
            if (action.type === 'catch_up_person') {
                ...
                results.push(`${person.name}的近况已更新。世界时间未推进。`);
                continue;
            }
            if (action.type === 'social_send_message') {
                const person = findLingqiPersonTarget(action);
                if (!person || person.isUser) {
                    results.push('无法确认目标通讯人物，消息仍保留在待确认状态，未发送。');
                    continue;
                }
                ...
                results.push(sent?.replyCount
                    ? `消息已发送给 ${person.name}，收到 ${sent.replyCount} 条回复。`
                    : `消息已发送给 ${person.name}，本次未收到回复。`);
                continue;
            }
            if (['social_accept_request', 'social_refuse_request'].includes(action.type)) {
                const person = findLingqiPersonTarget(action);
                if (!person || person.isUser) {
                    results.push('好友申请的人我没认准，先用操作按住，没有乱处理。');
                    continue;
                }
            ...
            if (action.type === 'social_refresh_moments') {
                ...
                results.push(refreshed?.postCount
                    ? `我从朋友圈窗口蹲回已到达。看到 ${refreshed.postCount} 条新动态${refreshed.imageCount ? '，其中一条还带了配图' : ''}。`
            ...
            if (action.type === 'refresh_public_world') {
                ...
                results.push(['queued', 'pending'].includes(runtime.publicOpinionStatus?.phase)
                    ? '本轮任务已进入队列。当前世界任务结束后将继续执行，不会重复创建。'
                    : '公共世界检查已完成；已结算可确认的变化与舆情，没有实际变化时不会强行生成内容。');
                continue;
            }
...
function dismissLingqiPendingAction() {
    ...
    runtime.pendingLingqiAction = null;
    runtime.lingqiStatus = { phase: 'idle', message: '操作已取消。', error: '' };
    runtime.ui?.render();
    return true;
}
...
function resolveLingqiLocalQuery(request, { state, settings, sync, clock }) {
    ...
    if (request.type === 'list_people') {
        ...
        if (people.length > shown.length) shown.push(`· 还有 ${people.length - shown.length} 个，可通过人物名称进行精确查询。`);
    ...
    if (request.type === 'diagnose_person') {
        ...
        const diagnosis = reasons.length
            ...
            : '现有开关和任务状态不足以确定原因；该人物可能尚未达到结算条件，或当前阶段确实没有状态变化。';
        return {
    ...
    if (request.type === 'recent_events') {
        ...
        if (!events.length) return { reply: '当前没有活跃暗流；系统不会为填充列表而生成无依据事件。', mascotState: 'idle', actions: [] };
    ...
    if (request.type === 'social_overview') {
        ...
        return { reply: `我把通讯角落巡了一圈，获得来的情况是：\n${lines.join('\n')}`, mascotState: unread.length || incoming.length ? 'watch' : 'idle', actions: [] };
    }
    if (request.type === 'social_person_status') {
        const person = findLingqiPersonTarget(request);
        if (!person || person.isUser) return { reply: '无法确认你指的是哪位人物，因此未对人物关系作出推断。', mascotState: 'confused', actions: [] };
        const social = normalizeSocialState(getStore().social || emptySocialState(), state.people || []);
        ...
        return { reply: `${person.name} 的当前关系为：${label}${evidence ? `\n相关记录：${evidence}` : ''}`, mascotState: 'watch', actions: [] };
    }
...
function resolveLingqiLocalButlerRequest(userText = '') {
    ...
    const socialSendMatch = null;
    if (socialSendMatch) {
        return { reply: '该操作会实际发送消息。已将收件人与原文放入确认卡，确认后再发送。', mascotState: 'watch', actions: [{ type: 'social_send_message', personName: socialSendMatch[1].trim(), text: socialSendMatch[2].trim() }] };
    }
    const socialRelationMatch = null;
    if (socialRelationMatch) {
        ...
        return { reply: '关系变更需要确认。先核对人物与当前关系，再执行确认操作。', mascotState: 'hold', actions: [{ type, personName: socialRelationMatch[2].trim() }] };
    }
    ...
    if (catchUpPersonMatch) {
        return {
            reply: `将补算 ${catchUpPersonMatch[1].trim()} 的近期状态。该操作会调用一次后台模型，需要确认后执行。`,
            mascotState: 'watch',
    ...
    const directSettingCommands = [
        ...
        {
            re: /^(?:帮我|帮忙|请|给我|直接)?\s*(?:整理|整理|归档)(?:一下|下)?\s*(?:长期)?记忆\s*[吧。！!]*$/iu,
    ...
    const promise = (async () => {
        try {
            ...
            const fallbackHelpLine = triage.route === 'mama'
                && !/(维护者|帖子|看不出来|不知道|查不明白|求助)/u.test(response.reply)
                ? '当前信息不足以定位问题，已生成可复制的诊断信息。'
                : '';
            const finalReply = [
                response.reply,
                actionReply,
...
async function sendSocialMessage(conversationId, messageText, { requestOnly = false } = {}) {
    ...
    if (requestOnly) {
        ...
        if (getSettings().socialInstantReply === false) {
            runtime.socialStatus = {
                phase: 'success',
                message: '消息已发送。本次未调用 API；如需立即生成回复，请使用相应的回复操作。',
                error: '',
                conversationId,
            };
            runtime.ui?.render();
...
async function handleUiAction(action, payload = {}) {
    ...
    if (action === 'bulk-delete-memory') {
        ...
        commitManualState(
            next,
            lockedSkipped
                ? `已删除 ${removed} 条记忆；${lockedSkipped} 条锁定记忆已保留。`
                : `已删除 ${removed} 条记忆；删除前的恢复点已保存。`,
            {
    ...
    if (action === 'clear-person-avatar') {
        ...
        commitManualState(next, '人物头像已恢复为文字头像。');
        return true;
    }
    ...
    if (['social_send_message', 'social_accept_request', 'social_refuse_request', 'social_remove_friend'].includes(action.type)) {
        const person = findLingqiPersonTarget(action);
        if (!person || person.isUser) return { error: '无法确认要操作的人物，因此未修改关系。' };
        if (action.type === 'social_send_message') {
...
function unregisterEvents() {
    for (const subscription of runtime.eventSubscriptions.splice(0)) {
        try {
            if (typeof subscription.source?.off === 'function') subscription.source.off(subscription.event, subscription.handler);
            else subscription.source?.removeListener?.(subscription.event, subscription.handler);
        } catch {}
    }
}
...
export function initializeWorldBackstage() {
    ...
    hostWindow.worldBackstageHost = {
        ...
        chatWithAssistant: text => sendLingqiMessage(text),
...
export function destroyWorldBackstage() {
    unregisterEvents();
    for (const key of ['publicOpinionTimer', 'publicImpactTimer', 'autoMemoryTimer', 'socialPulseTimer', 'manualSimulationTimer', 'manualUndoTimer']) {
        if (runtime[key] !== null && runtime[key] !== undefined) {
            try { window.clearTimeout(runtime[key]); } catch {}
            runtime[key] = null;
        }
    }
    for (const key of ['activeSimulation', 'activeHistoryScan', 'activeWorldPulse', 'activePublicImpact', 'activeCorrection', 'activePublicOpinion', 'activePublicOpinionSandbox', 'activeObservation', 'activeLingqi', 'activeSocial']) {
        try { runtime[key]?.controller?.abort?.(); } catch {}
        runtime[key] = null;
    }
