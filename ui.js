...
const TOAST_FACES = { success: '', busy: '', info: '', normal: '', warning: '', error: '' };

const TOAST_LABELS = { success: '完成', busy: '处理中', info: '提示', normal: '提示', warning: '注意', error: '错误' };
...
async function readPersonAvatarFile(file) {
    if (!file) return '';
    if (!String(file.type || '').startsWith('image/')) throw new Error('所选文件不是可识别的图片。');
    if (Number(file.size || 0) > 8 * 1024 * 1024) throw new Error('头像文件过大，请选择 8MB 以内的图片。');

    const source = await new Promise((resolve, reject) => {
        const reader = new FileReader();
        reader.onload = () => resolve(String(reader.result || ''));
        reader.onerror = () => reject(new Error('头像读取失败，请更换图片后重试。'));
        reader.readAsDataURL(file);
    });
    const image = await new Promise((resolve, reject) => {
        const node = new Image();
        node.onload = () => resolve(node);
        node.onerror = () => reject(new Error('浏览器无法识别该图片，请改用 PNG、JPG 或 WebP 格式。'));
        node.src = source;
    });
    ...
    if (result.length > 180000) throw new Error('头像压缩后仍超过大小限制，请更换图片。');
    return result;
}
...
function renderSettings(state, settings, syncStatus, openGroups = new Set(), openSubgroups = new Set(), apiDraft = null, tagFilterRules = null, tagCandidates = [], worldbookUi = {}, tavernProfiles = [], activeSection = 'common') {
    ...
    const settingExplanation = (setting, value) => {
        ...
        const maps = {
            ...
            uiScale: {
                compact: '减少间距与字号，在同一屏幕显示更多信息。',
                comfortable: '使用默认字号与间距，兼顾信息密度和可读性。',
                large: '增大字号和控件尺寸，提升小屏幕可读性。',
            },
            deliveryDensity: {
                restrained: '减少主动显露，后台模块仍正常运行。',
                balanced: '优先显露重要变化，其他信息保持后台。',
                active: '更积极地显露世界变化，同时避免在不合适的场景强行插入。',
            },
            autoSimulationMode: {

                light: '降低后台推演范围，只处理必要变化，减少模型调用和状态变动。',
                balanced: '在连续性、后台变化和资源消耗之间保持平衡。',
                deep: '扩大后台推演范围，更详细地处理人物、事件与因果关系，资源消耗更高。',
            },
            timePolicy: {
                world: '以世界钟为准；正文出现明确时间信息时同步世界时间。',
                explicit: '仅在正文提供明确、可计算的时间证据时推进世界时间。',
                cautious: '允许根据较明确的叙事线索估算时间；证据不足时不推进。',
                open: '允许根据旅行、等待、工作等明显耗时行为估算并推进世界时间。',
            },
            publicOpinionRevealMode: {
                observe: '舆情仅在面板中查看，不主动提供给正文。',
                relevant: '仅在与当前剧情相关时提供少量舆情信息。',
            },
        };
    ...
    return `
        <div class="wb-settings-popover wb-settings-scope-global wb-settings-section-${sectionKey}" role="dialog" aria-modal="true"
            ...
            <section class="wb-settings-section-body wb-settings-common-section">
                <div class="wb-global-flat-block">
                    <div class="wb-flat-section-heading">
                        <div><strong>外观</strong><span>设置主题、字号与界面显示方式。</span></div>
                    </div>
                ...
                <div class="wb-global-common-grid">
                    <div class="wb-setting-toggle">
                        <div>
                            ...
                            <span>关闭后停止新的世界推演，已有状态与历史数据保留。</span>
                        </div>
                    ...
                    <div class="wb-setting-toggle">
                        <div>
                            <strong>自动运行</strong>
                            <span>开启后会在新正文出现时自动安排世界推演；关闭后只手动运行。</span>
                        </div>
                        <label class="wb-switch">
            ...
            <div class="wb-setting-block">
                ...
                ${settings.apiMode === 'tavern-profile' ? `
                    ...
                    <p>${tavernApiProfiles.length
                        ? '这里只保存方案 ID；Key、接口地址和模型继续由 SillyTavern 管理。方案变更会在下次调用时读取，方案被删除时会明确报错。'
                        : '未读取到可用的酒馆连接方案。请先在 SillyTavern Connection Manager 中保存方案，再返回选择。'}</p>
                ` : ''}
            </div>
                    </div>
                ...
                <form class="wb-api-form" data-wb-form="api" autocomplete="off">
                    ...
                    <div class="wb-api-draft-heading">
                        <span>${apiValues.profileId ? '正在编辑已保存方案；API Key 留空将继续使用原值。' : (hasSavedApiKey ? '已保存默认独立接口；出于安全考虑，已有 API Key 不会回填显示。' : '可先填写接口进行测试；常用配置可保存为方案供不同模块复用。')}</span>
                    ...
                    <p>接口地址填写到版本层即可，例如 <code>/v1</code>；<code>/chat/completions</code> 会自动补全。</p>
                    <label>API Key
                    ...
                    <p>${hasSavedApiKey
                        ? '填写新 Key 将覆盖旧值；留空则继续使用已保存 Key。旧 Key 不会回填到输入框。'
                        : 'API Key 仅保存在本机 SillyTavern 设置中，不写入导出的世界状态。'}</p>
                    <label>模型名称
                    ...
                    <details class="wb-settings-subgroup" data-settings-subgroup="connection-profiles" ${subgroupOpen('connection-profiles')}>
                        ...
                        <div class="wb-settings-subgroup-body">
                            ...
                            ` : '<p>常用接口可保存为方案，之后可直接分配给不同模块，无需重复填写 URL 和 Key。</p>'}
                        </div>
                    </details>
            ...
            <div class="wb-setting-toggle">
                <div><strong>正文注入总开关</strong><span>关闭后仅停止向正文提供这些信息；人物、事件、记忆和舆情仍可继续后台运行。</span></div>
                <label class="wb-switch">
                    <input type="checkbox" data-wb-setting="worldPromptInjection"
                        ${settings.worldPromptInjection ? 'checked' : ''}>
                    <i></i>
            ...
            <div class="wb-injection-grid ${settings.worldPromptInjection ? '' : 'is-disabled'}">
                <div class="wb-setting-block wb-injection-time">
                    ...
                    <p class="wb-setting-explanation">「最小锚点」只提供维持时间连续性所需的信息，不要求正文显式报时。</p>
                </div>
                <div class="wb-setting-toggle"><div><strong>「此刻」· 世界背景</strong><span>世界背景始终保留；这里只控制是否提供给正文。</span></div><label class="wb-switch"><input type="checkbox" data-wb-setting="injectionWorldBackground" ${settings.injectionWorldBackground !== false ? 'checked' : ''}><i></i></label></div>
                <div class="wb-setting-toggle"><div><strong>「人物」· 当前状态</strong><span>人物状态照常保存；这里只控制是否提供给正文。</span></div><label class="wb-switch"><input type="checkbox" data-wb-setting="injectionPeople" ${settings.injectionPeople !== false ? 'checked' : ''}><i></i></label></div>
                <div class="wb-setting-toggle"><div><strong>「暗流」· 进行中事件与世界环境</strong><span>镜头外事件照常推进；这里只控制是否提供给正文。</span></div><label class="wb-switch"><input type="checkbox" data-wb-setting="injectionEvents" ${settings.injectionEvents !== false ? 'checked' : ''}><i></i></label></div>
                <div class="wb-setting-toggle"><div><strong>「回声」· 已结算后果</strong><span>只控制是否提供给正文，不会删除已结算后果。</span></div><label class="wb-switch"><input type="checkbox" data-wb-setting="injectionEchoes" ${settings.injectionEchoes !== false ? 'checked' : ''}><i></i></label></div>
                <div class="wb-setting-toggle"><div><strong>「此刻」· 世界事实</strong><span>已成立事实用于一致性校验。</span></div><label class="wb-switch"><input type="checkbox" data-wb-setting="injectionFacts" ${settings.injectionFacts !== false ? 'checked' : ''}><i></i></label></div>
                <div class="wb-setting-toggle"><div><strong>「记忆」· 长期记忆</strong><span>关闭后仍会整理记忆，只是不提供给正文。</span></div><label class="wb-switch"><input type="checkbox" data-wb-setting="injectionMemory" ${settings.injectionMemory !== false ? 'checked' : ''}><i></i></label></div>
                <div class="wb-setting-toggle"><div><strong>「舆情」· 新闻与论坛</strong><span>舆情仍可继续更新；这里只控制是否提供给正文。</span></div><label class="wb-switch"><input type="checkbox" data-wb-setting="injectionPublicOpinion" ${settings.injectionPublicOpinion !== false ? 'checked' : ''}><i></i></label></div>
            </div>
            <p class="wb-setting-explanation">这些开关只控制是否提供给正文，不影响对应后台数据是否存在或继续运行。</p>
            ...
            <details class="wb-settings-group" data-settings-group="simulation" ${groupOpen('simulation')}>
                ...
                <div class="wb-settings-group-body wb-settings-subgroup-stack">
                    <details class="wb-settings-subgroup" data-settings-subgroup="simulation-switches" ${subgroupOpen('simulation-switches')}>
                        ...
                        <div class="wb-settings-subgroup-body">
            <div class="wb-setting-toggle">
                <div><strong>启用世界引擎</strong><span>关闭后停止新的世界推演，已有世界状态保留。</span></div>
                <label class="wb-switch">
                    <input type="checkbox" data-wb-setting="worldSimulationEnabled"
                        ${settings.worldSimulationEnabled ? 'checked' : ''}>
                    <i></i>
            ...
            <div class="wb-setting-block">
                ...
                <p class="wb-setting-explanation">可自定义单轮后台人物数量。建议 2～8 人；数量越高，Token 消耗和等待时间通常越大。</p>
            </div>

            <div class="wb-setting-toggle">
                <div><strong>强化后台人物推演</strong><span>将到期的后台人物纳入现有世界推演，不额外为每个人单独发送请求。</span></div>
                <label class="wb-switch">
            ...
            <div class="wb-setting-toggle">
                <div><strong>记录玩家角色</strong><span>开启后记录玩家在正文中明确发生的位置、行动和客观状态；关闭后不在「人物」中维护玩家状态，但已发生的正文事实仍保留。</span></div>
                <label class="wb-switch">
            ...
            <div class="wb-setting-block">
                ...
                <p class="wb-setting-explanation">${escapeHtml(
                    [1, 3, 5].includes(Number(settings.contextTurns))
                        ? ({1: '只读取最新一轮，消耗最低，适合当前信息明确的场景。', 3: '读取最近 3 轮，在连续性与消耗之间较为均衡。', 5: '默认推荐。最近 5 轮通常足以维持人物与事件连续性。'}[Number(settings.contextTurns)])
                        : `现在会读最近 ${settings.contextTurns} 轮。长事件和多人剧情通常更稳定，但读取范围越大，Token 消耗也越高。`
                )}</p>
            </div>
                        ...
                        <label>选择世界书
                            <select name="bookName" ${worldbookBooks.length ? '' : 'disabled'}>
                                ${worldbookBooks.length
                                    ...
                                    : '<option value="">未找到可读取的世界书</option>'}
                            </select>
                        </label>
            ...
            <details class="wb-settings-group" data-settings-group="memory" ${groupOpen('memory')}>
                ...
                <div class="wb-settings-group-body wb-settings-subgroup-stack">
                    <details class="wb-settings-subgroup" data-settings-subgroup="memory-switches" ${subgroupOpen('memory-switches')}>
                        ...
                        <div class="wb-settings-subgroup-body">
            <div class="wb-setting-toggle">
                <div><strong>启用记忆系统</strong><span>关闭后不再整理新记忆，已保存记忆保持不变。</span></div>
                <label class="wb-switch">
            ...
            <p class="wb-setting-explanation">是否把记忆提供给正文请在「正文注入」设置；这里仅控制记忆系统是否继续整理新内容。</p>
                        </div>
                    </details>
    ...
    : Number(memory.pendingAssistantResponses || 0) > 0
        ...
        : memory.summaryBehind
            ...
            : memory.pendingRollup
                ...
                ` : ''}
                <p>自动整理新增正文中的重要事实、关系、承诺与未完成伏笔。</p>
            ...
            <details class="wb-settings-group" data-settings-group="calendar" ${groupOpen('calendar')}>
                ...
                <div class="wb-settings-group-body">
                    <form class="wb-clock-form" data-wb-form="clock">
                        ...
                        <p class="wb-clock-sync-note">正文出现明确时间时会自动同步；也可在此手动校准当前世界时间。</p>
            ...
            <details class="wb-settings-group wb-settings-advanced-section" data-settings-group="advanced" ${sectionKey === 'advanced' ? 'open' : groupOpen('advanced')}>
                <summary><span>高级与维护</span><small>高级参数、备份与维护工具</small></summary>
                <div class="wb-settings-group-body wb-advanced-settings-body wb-settings-subgroup-stack">
                    <details class="wb-settings-subgroup" data-settings-subgroup="advanced-generation" ${subgroupOpen('advanced-generation')}>
                        ...
                        <div class="wb-settings-subgroup-body">
                            <div class="wb-setting-block">
                                ...
                                <p class="wb-setting-explanation">设为 0 时自动分配输出预算；服务端限制更低时以服务端为准。</p>
                            </div>
                            ...
                            <div class="wb-setting-block">
                                ...
                                <p class="wb-setting-explanation">设为 0 时自动分配超时；仅统计模型实际生成时间，不包含限流冷却和排队时间。</p>
                            </div>
                    ...
                    <details class="wb-settings-subgroup" data-settings-subgroup="advanced-data" ${subgroupOpen('advanced-data')}>
                        <summary><span>数据备份</span><small>导出当前世界状态备份</small></summary>
                        <div class="wb-settings-subgroup-body">
                        ...
                        <div class="wb-settings-subgroup-body">
                            <div class="wb-maintenance-card">
                                <div>
                                    ...
                                    <p>仅清理可重建缓存，不删除人物、记忆和世界状态。</p>
                                </div>
                                <button type="button" data-wb-action="clear-current-chat-cache">清理缓存</button>
                            </div>
                            <div class="wb-maintenance-card">
                                <div>
                                    <strong>检查世界状态</strong>
                                    <p>根据正文与明确事实校正后台状态，不修改已经存在的正文。</p>
                                </div>
                    ...
                    <details class="wb-settings-subgroup" data-settings-subgroup="advanced-tagfilter" ${subgroupOpen('advanced-tagfilter')}>
                        <summary><span>正文过滤</span><small>过滤变量块、脚本块等非叙事内容，只把有效叙事文本提供给后台。</small></summary>
                        <div class="wb-settings-subgroup-body">
                    <div class="wb-setting-toggle">
                        <div><strong>启用标签过滤</strong><span>关闭后不再应用自定义标签过滤；HTML 注释仍会按安全规则排除。</span></div>
                        <label class="wb-switch">
                    ...
                    <div class="wb-setting-block">
                        ...
                        <p>HTML 注释会整体移除；下列成对标签也可直接排除，无需编写正则表达式。</p>
                        ...
                        <div class="wb-tag-auto-tools">
                            <div class="wb-tag-auto-head">
                                <div><strong>自动提取候选</strong><span>仅列出检测到的候选标签，不会自动加入过滤规则。</span></div>
                    ...
                    <details class="wb-settings-subgroup" data-settings-subgroup="advanced-recovery" ${subgroupOpen('advanced-recovery')}>
                        <summary><span>安全恢复</span><small>${latestRecovery ? escapeHtml(latestRecovery.label) : '最近的世界状态恢复点'}</small></summary>
                        <div class="wb-settings-subgroup-body">
                            <div class="wb-maintenance-status">
                                <strong>${latestRecovery ? escapeHtml(latestRecovery.label) : '还没有恢复点'}</strong>
                                <span>${latestRecovery ? escapeHtml(formatLocalTimestamp(latestRecovery.createdAt)) : '恢复点按聊天独立保存'}</span>
                            </div>
                            <p>${latestRecovery
                                ? `当前保存 ${Math.max(1, Number(recovery.count) || 1)} 个恢复点，恢复后仍会先替现在的状态留一份保险。`
                                : '升级旧数据或导入世界状态前会创建恢复点，也可随时手动创建。'}</p>
                            <div class="wb-setting-actions">
                    ...
                    <details class="wb-settings-subgroup" data-settings-subgroup="advanced-diagnostics" ${subgroupOpen('advanced-diagnostics')}>
                        <summary><span>故障诊断</span><small>生成排障需要的信息</small></summary>
                        <div class="wb-settings-subgroup-body">
                            <p>仅导出版本、设备、接口模式和错误状态；不会包含 API Key、接口地址、正文或角色私密设定。</p>
                            <div class="wb-setting-actions">
...
function renderModuleSettings(state, settings, syncStatus, scope = 'now', openSubgroups = new Set(), worldbookUi = {}) {
    const scopeMeta = {
        now: { eyebrow: '当前模块', title: '此刻设置' },
        people: { eyebrow: '当前模块', title: '人物设置' },
        currents: { eyebrow: '当前模块', title: '暗流设置' },
        opinion: { eyebrow: '当前模块', title: '舆情设置' },
        memory: { eyebrow: '当前模块', title: '记忆设置' },
    };
    ...
    const explanation = (setting, value) => ({
        ...
        worldPulseActivity: {
            quiet: '降低公共世界变化频率，适合希望背景更稳定的剧情。',
            natural: '按常规频率检查城市、组织、天气和社会变化。',
            busy: '提高公共世界变化检查频率，但不会无依据制造重大事件。',
        },
    ...
    const injectionStatusCard = (label, enabled, detail = '') => `
        <div class="wb-module-visibility-card ${enabled ? 'is-on' : 'is-off'}">
            <div>
                <span>是否注入正文</span>
                <strong>${escapeHtml(label)} · ${enabled ? '会注入正文' : '先暂不注入'}</strong>
    ...
    const nowHtml = `
        ...
        <div class="wb-flat-setting-list">
            <div class="wb-setting-block">
                ...
                <p class="wb-setting-explanation">${escapeHtml(
                    [1, 3, 5].includes(Number(settings.contextTurns))
                        ? ({1: '只读取最新 1 轮上下文，Token 消耗最低。', 3: '读取最近 3 轮上下文，适合多数日常剧情。', 5: '读取最近 5 轮上下文，在连续性和 Token 消耗之间平衡。'}[Number(settings.contextTurns)])
                        : `当前读取最近 ${settings.contextTurns} 轮上下文；轮数越高连续性通常越好，但 Token 消耗也会增加。`
                )}</p>
            </div>
                ...
                <div class="wb-world-background-body">
                    <div class="wb-world-background-preview ${worldBackground ? 'is-set' : 'is-empty'}">
                        ${escapeHtml(worldBackground
                            ...
                            : '填写长期成立的时代、地理、势力、规则和重要时间线，作为世界推演的稳定背景。')}
                    </div>
        ...
        ${sectionHeading('世界钟', '查看或校准当前主世界时间')}
        <form class="wb-clock-form wb-flat-clock-form" data-wb-form="clock">
            <div class="wb-clock-form-heading"><div><label>主世界日历</label><strong>${escapeHtml(clockLabel)}</strong></div><span>世界时间按聊天独立保存</span></div>
    ...
    const peopleHtml = `
        ...
        <div class="wb-person-group-life-note">
            <span>群体生活 · 默认生效</span>
            <strong>同地点、同行、同组织和共同事件的人物会优先放在同一轮中统一结算。</strong>
        ...
        <div class="wb-flat-setting-list">
            ...
            <div class="wb-setting-toggle">
                <div><strong>强化后台人物推演</strong><span>开启后，将到期人物纳入同一轮世界推演统一结算，减少重复调用。</span></div>
            ...
            <div class="wb-setting-toggle">
                <div><strong>记录玩家角色</strong><span>关闭后不单独为玩家角色建立后台人物状态；正文中明确发生的客观事实仍会被记录。</span></div>
            ...
            ${routeSetting('observation', '人物观测使用的连接', '为人物观测指定连接；Key 和地址仍由全局连接配置管理。')}
        </div>
        <details class="wb-settings-subgroup wb-module-advanced" data-settings-subgroup="people-worldbook" ${advancedOpen('people-worldbook')}>
            <summary><span>世界书人物导入</span><small>从世界书筛选并导入人物资料。</small></summary>
            <div class="wb-module-advanced-body">
                <form class="wb-worldbook-import" data-wb-form="worldbook">
                    ...
                    <p class="wb-setting-explanation">导入时会跳过 MVU、变量与脚本类条目，并尽量识别真实人物，避免把组织或技术条目误判成人物。</p>
                    ${worldbook.message ? `<div class="wb-worldbook-status is-${escapeAttr(worldbook.phase)}">${escapeHtml(worldbook.message)}</div>` : ''}
                    ${worldbookEntries.length ? `
                        <div class="wb-worldbook-browser">
                            ...
                            <div class="wb-worldbook-entry-list">
                                ...
                                }).join('') : '<div class="wb-worldbook-empty">未找到符合条件的条目，请调整关键词或筛选条件。</div>'}
                            </div>
    ...
    const currentsHtml = `
        ${injectionStatusCard(
            ...
            `回声：${settings.worldPromptInjection && settings.injectionEchoes !== false ? '已注入' : '未注入'} · 世界事实：${settings.worldPromptInjection && settings.injectionFacts !== false ? '已注入' : '未注入'}`,
        )}
        ${sectionHeading('世界运行', '控制镜头外世界的持续推演')}
        <div class="wb-flat-setting-list">
            <div class="wb-module-runtime-summary">
                <div><span>自动运行</span><strong>${settings.worldAutoEnabled ? '自动运行' : '仅手动触发'}</strong></div>
            ...
            ${routeSetting('simulation', '世界推演使用的连接', '为世界推演指定连接；Key 和地址由全局连接配置管理。')}
        </div>
        <details class="wb-settings-subgroup wb-module-advanced" data-settings-subgroup="currents-advanced" ${advancedOpen('currents-advanced')}>
            <summary><span>高级</span><small>失败处理与重试策略</small></summary>
    ...
    const opinionHtml = `
        ${injectionStatusCard(
            ...
            '舆情模块可继续后台更新；此处只控制是否向正文模型提供舆情。',
        )}
        <div class="wb-flat-setting-list">
            <div class="wb-setting-toggle">
                <div>
                    <strong>自动更新舆情</strong>
                    <span>开启后按条件自动更新舆情；关闭后仍可手动刷新。</span>
                </div>
                <label class="wb-switch">
            ...
            ${routeSetting('opinion', '舆情更新使用的连接', '为舆情更新指定连接；接口参数由全局连接配置管理。')}
        </div>
        <p class="wb-flat-footnote">公开新闻会把真实影响接回人物和环境，不过要先对上世界事件，再顺着地点、工作、组织和消息渠道影响该碰到的人；论坛传闻仍属于未证实信息，不会自动转化为所有人物已知的世界事实。</p>
    `;
    ...
    const memoryHtml = `
        ...
        <div class="wb-flat-setting-list">
            <div class="wb-setting-toggle">
                <div><strong>启用记忆系统</strong><span>关闭后停止整理新记忆，已保存的长期记忆保持不变。</span></div>
            ...
            ${routeSetting('history', '记忆整理使用的连接', '为记忆整理指定连接；Key 和地址由全局连接配置管理。')}
        </div>
        ${sectionHeading('中途接入', '从已有聊天记录回溯建立世界状态')}
        <div class="wb-history-settings wb-flat-history-settings wb-world-bootstrap-settings">
            <div class="wb-history-heading">
                <div>
                    ...
                    <strong>${escapeHtml(historyRunning && memory.kind === 'world-bootstrap'
                        ? memory.message || '正在分批回溯已有聊天记录。'
                        : '重新整理时间、人物、事实、未完成事件与记忆')}</strong>
                </div>
            ...
            ${historyRunning && memory.kind === 'world-bootstrap'
                ? `<div class="wb-history-progress"><i style="width:${historyPercent}%"></i></div>`
                : ''}
            <p>每个批次都会保存进度；中断后可从上次进度继续。全部完成前不会用半成品覆盖正式世界状态。</p>
            <button type="button" data-wb-action="bootstrap-history" ${historyRunning ? 'disabled' : ''}>回溯当前聊天</button>
        </div>

        ${sectionHeading('记忆整理', '仅整理长期记忆，不触发完整世界推演')}
        <div class="wb-history-settings wb-flat-history-settings">
            <div class="wb-history-heading"><div><label>长期记忆</label><strong>${escapeHtml(historyRunning && memory.kind !== 'world-bootstrap' ? memory.message || '正在整理长期记忆。' : '长期记忆将按设置自动整理。')}</strong></div>
                <span>${historyRunning && memory.kind !== 'world-bootstrap' ? `${historyPercent}%` : (Number(memory.pendingAssistantResponses || 0) > 0 ? '存在待整理的新正文。' : '长期记忆已同步至最新正文。')}</span></div>
            ${historyRunning && memory.kind !== 'world-bootstrap' ? `<div class="wb-history-progress"><i style="width:${historyPercent}%"></i></div>` : ''}
            <p>系统会提取重要事实、关系、承诺与未完成线索，并按层级压缩以控制记忆规模。</p>
            ...
            <button type="button" data-wb-action="scan-history" ${historyRunning || !settings.memorySystemEnabled ? 'disabled' : ''}>整理长期记忆</button>
        </div>
    `;
        ...
        <div class="wb-drawer-scrim" data-wb-action="close-event-form">
            <form class="wb-event-form" data-wb-form="event">
                ...
                ${isEdit ? `
                    ...
                    <p class="wb-form-hint">手动状态是当前世界事实；标记为已结束 / 已取消 / 已错过后，后台推演不会把这条旧暗流自动改回发展中。若后来真的出现新后果，应作为新的暗流继续。</p>
                ` : ''}
                <div class="wb-form-grid">
                    <label>计时方式
            ...
            <div class="wb-lingqi-surface">
                <section class="wb-lingqi-chat-card">
                                ...
                                ${message.role !== 'user' && message.needsAuthorHelp ? `
                                    <div class="wb-lingqi-support-card">
                                        ...
                                        ${message.supportTriage?.checked?.length ? `
                                            <small>已检查：${escapeHtml(message.supportTriage.checked.join('；'))}</small>
                                        ` : ''}
                    ...
                    ${pending ? `
                        <div class="wb-lingqi-proposal">
                            ...
                            <div class="wb-lingqi-proposal-actions">
                                <button type="button" data-wb-action="lingqi-confirm-note">保留</button>
                                <button type="button" data-wb-action="lingqi-dismiss-note">放弃</button>
                            </div>
                        </div>
                    ` : ''}
                    ${pendingAction ? `
                        <div class="wb-lingqi-action-confirm" role="alertdialog"
                            ...
                            <div class="wb-lingqi-action-confirm-buttons">
                                ...
                                <button type="button" class="is-confirm" data-wb-action="lingqi-confirm-action">
                                    ${escapeHtml(pendingAction.confirmLabel || '确认')}
                                </button>
                            </div>
                        </div>
                    ` : ''}
                    <form class="wb-lingqi-input" data-wb-form="lingqi-chat">
                        ...
                        <button type="submit" aria-label="发送" title="发送" ${lingqiBusy ? 'disabled' : ''}>${lingqiBusy ? '…' : '发送'}</button>
                    </form>
                </section>

                <section class="wb-lingqi-notes-card ${notesEmpty ? 'is-empty' : ''}">
                <div class="wb-lingqi-notes-head">
                    <div><span>◇</span><strong>剧情引导</strong></div>
                        ...
                        <article class="wb-lingqi-note is-${escapeAttr(note.status)} is-tone-${index % 4}" style="--wb-note-tilt:${index % 2 ? '0.8deg' : '-0.7deg'}">
                            ...
                            <div class="wb-lingqi-note-foot">
                                ...
                                <div>
                                    ...
                                    `}
                                    <button type="button" data-wb-action="lingqi-cancel-note" data-note-id="${escapeAttr(note.id)}">删除</button>
                                </div>
                            </div>
                        </article>
                    `).join('') : `
            ...
            <form class="wb-event-form wb-person-editor" data-wb-form="person">
                ...
                <div class="wb-person-avatar-editor">
                    ...
                    <div>
                        ...
                        <small>会自动裁成 160×160 的小头像保存，不会塞进模型上下文 </small>
                    </div>
                ...
                <label>当前幕后独白<textarea name="innerVoice" maxlength="240" rows="3"
                    ...
                    <small>这是当前心声，不是永久人设锁。写歪了就直接掰回来 </small>
                </label>
    ...
    return `
        ...
        <div class="wb-event-list is-full">
            ${activeEvents.map(event => renderEventCard(event, state, true, openFolds)).join('')
                || renderEmpty('当前没有进行中的暗流。', '到期、取消或错过的事件会离开暗流列表，已形成的结果会进入回声。')}
        </div>
    `;
}
    ...
    return `
        <div class="wb-opinion-toolbar" aria-busy="${running ? 'true' : 'false'}">
            <div class="wb-opinion-summary">
                <div class="wb-opinion-meta">
                    ...
                    ${recentWorldCoverageLabels.length
                        ...
                        : ''}
                    ${stale
                        ? '<span class="is-stale">世界状态已更新 · 建议刷新舆情</span>'
            ...
            <div class="wb-opinion-actions">
                ...
                <button class="wb-inline-add" type="button" data-wb-action="generate-public-opinion" ${canonRunning ? 'disabled' : ''}
                    title="世界时间推进达到条件、首次升级或关注重点变化时会检查公共世界；没有实际变化时不会仅为改写措辞重复请求">
        ...
        ${activeMode === 'news' ? `
            <div class="wb-news-grid">
                ...
                `).join('') || renderEmpty('今天的世界有点安静。', '当前没有可展示的新闻，或尚未生成舆情快照。')}
            </div>
        ` : activeMode === 'forum' ? `
        ...
        ` : `
            <div class="wb-opinion-sandbox">
                ...
                ${sandboxItems.length ? '' : renderEmpty('今天还没随便逛。', '点一下“随便逛逛。”，生成一组与主线无关的非正史小新闻和论坛话题。')}
            </div>
        `}
    `;
}
...
function memorySummaryLevelMeta(summary) {
    ...
    const meta = [
        ...
        { label: '阶段小结', tone: 'stage', description: '记录一个剧情阶段的主要信息。' },
        { label: '章节经历', tone: 'chapter', description: '把更长一段经历收成重点。方便后面继续接上。' },
        { label: '长期经历', tone: 'longterm', description: '跨越较长时间后仍重要的变化会保留在这里。' },
    ];
    return meta[level];
}
    ...
    return `
        <div class="wb-drawer-scrim" data-wb-action="close-world-editor">
            <form class="wb-event-form wb-world-editor" data-wb-form="world">
                ...
                <div class="wb-form-note">
                    这里是世界的“地基”。后台人物、World Pulse、公共事件和正文一致性都会参考它；普通推演不会自己改写。适合写长期规则和重要时间线，不用把整张角色卡都塞进来 
                </div>
...
export function createWorldBackstageUI({
    ...
    function apiRequestFromDraft(data, { requireModel = true } = {}) {
        ...
        if (!key) throw new Error('这个接口还缺 API Key 。');
    ...
    function render() {
        ...
        const captionText = memoryPhase === 'error'
            ...
            : memoryPhase === 'running'
                ? syncStatus.memory.message || '记忆正在悄悄收拾中。 '
                : orbProcessing
                    ? syncStatus.message || '镜头外的世界正在悄悄运转中… ✧'
                    : syncStatus.phase === 'error'
                        ? '本轮正文尚未推演成功，可以在这里重试。'
                        : narrativePromptVisible
        ...
        root.innerHTML = `
                    ...
                    <section class="wb-window" role="dialog" aria-modal="true" aria-label="世界动态">
                        ...
                        <div class="wb-window-body">
                            <nav class="wb-side-nav">
                                ${VIEWS.filter(view => view.id !== 'lingqi').map(view => `
            ...
            ` : ''}
            ${false && settings.enabled && socialNotice ? `
    ...
    root.addEventListener('click', async event => {
        ...
        if (action === 'delete-record') {
            ...
            const confirmed = globalThis.confirm?.(`  确定删除这条${kind === 'echo' ? '回声' : '纪事'}吗？删除后可以用底部撤销恢复。`);
        ...
        if (action === 'clear-public-opinion') {
            const confirmed = globalThis.confirm?.('  清空当前舆情快照吗？世界状态不会受到影响。');
            if (confirmed === false) return;
        ...
        if (action === 'social-respond-friend-request') {
            ...
            if (!accept) {
                const confirmed = globalThis.confirm?.('  要拒绝这条好友申请吗？对方会保留自己的反应，但不会进入通讯录。');
                if (confirmed === false) return;
            }
        ...
        if (action === 'clear-person-avatar') {
            ...
            if (completed) notify('头像已过期。又变回文字头像了 ', 'success');
            render();
            return;
        }
        ...
        if (action === 'delete-event') {
            ...
            const confirmed = globalThis.confirm?.('  确定删除这条暗流吗？删除后可以用底部撤销恢复。');
            if (confirmed === false) return;
        ...
        if (action === 'delete-api-profile') {
            ...
            const confirmed = globalThis.confirm?.(`  要删掉 API 方案“${profile?.name || '这个方案'}”吗？\n使用它的模块会自动退回“跟随默认”。`);
            if (confirmed === false) return;
        ...
        if (action === 'clear-current-chat-cache') {
            ...
            if (completed) {
                personObservation = null;
                worldbookSelectedIds = new Set();
                notify('当前聊天的可重建缓存已清理；世界数据与 API 配置未修改。', 'success');
            }
            render();
            return;
        }
    ...
    root.addEventListener('change', async event => {
        ...
        if (event.target.matches?.('[data-wb-tavern-profile-select]')) {
            ...
            notify(event.target.value ? '酒馆连接方案已保存。' : '已取消绑定酒馆连接方案。', event.target.value ? 'success' : 'info');
            render();
            return;
        }
    ...
    root.addEventListener('submit', async event => {
        ...
        if (form.dataset.wbForm === 'api') {
            ...
            if (completed) {
                forgetApiKeyDraft(data);
                notify('独立接口已保存；已有 API Key 不会回填显示。', 'success');
            }
        }
