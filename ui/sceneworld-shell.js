...
.editor-backdrop{position:fixed;inset:0;background:rgba(48,48,42,.34);display:grid;place-items:center;padding:16px;z-index:30}.editor-dialog{width:min(560px,94vw);max-height:min(760px,90dvh);overflow:auto;border:1px solid var(--sw-line);border-radius:16px;background:var(--sw-card);box-shadow:var(--sw-shadow);padding:17px}.editor-dialog h2{margin:0 0 5px;font-size:calc(17px + var(--sw-font-adjust))}.editor-dialog>p{margin:0 0 13px;color:var(--sw-muted);font-size:calc(12px + var(--sw-font-adjust));line-height:1.6}.editor-field{display:block;margin-top:11px}.editor-field>span{display:block;font-size:calc(11px + var(--sw-font-adjust));color:var(--sw-muted);font-weight:650;margin-bottom:5px}.editor-field input,.editor-field textarea{width:100%;border:1px solid #d2cfc1;background:#fffef9;color:var(--sw-text);border-radius:9px;padding:9px 10px;outline:none}.editor-field textarea{min-height:92px;resize:vertical;line-height:1.55}.editor-field input:focus,.editor-field textarea:focus{border-color:#9eaa95;box-shadow:0 0 0 2px rgba(102,118,92,.10)}.editor-actions{display:flex;justify-content:flex-end;gap:8px;margin-top:15px}
@media(max-width:620px){.maintenance-grid{grid-template-columns:1fr}.editor-backdrop{padding:10px}.editor-dialog{width:100%;max-height:88dvh}.card-header .action{min-width:70px}.settings-tabs button{flex-basis:64px}}
`;
...
export function createSceneWorldShell({ version, onClose, actions }) {
    ...
    const openPersonEditor=person=>{
        ...
        const overlay=document.createElement('div');overlay.className='editor-backdrop';
        overlay.innerHTML=`<section class="editor-dialog" role="dialog" aria-modal="true" aria-label="${person?'修改人物':'添加人物'}"><h2>${person?'修改人物':'添加人物'}</h2><p>手动内容会作为当前人物状态保存，并参与后续世界推演。附加详情每行使用“标签：内容”。</p><label class="editor-field"><span>人物名称</span><input type="text" data-person-name value="${escapeHtml(person?.name||'')}"></label><label class="editor-field"><span>当前位置</span><input type="text" data-person-location value="${escapeHtml(person?.location||'')}"></label><label class="editor-field"><span>当前状态</span><textarea data-person-status>${escapeHtml(person?.status||'')}</textarea></label><label class="editor-field"><span>附加详情</span><textarea data-person-details placeholder="当前行动：……\n身体状态：……">${escapeHtml(personToDetailsText(person))}</textarea></label><label class="editor-field"><span>已确认认知</span><textarea data-person-knowledge placeholder="每行一条">${escapeHtml(personToKnowledgeText(person))}</textarea></label><div class="editor-actions"><button class="action" type="button" data-person-cancel>取消</button><button class="action primary" type="button" data-person-save>保存</button></div></section>`;
        ...
        shadow.appendChild(overlay);overlay.querySelector('[data-person-name]')?.focus();
    };
    const render=(options={})=>{
        ...
        shadow.querySelector('[data-action="refresh-street"]')?.addEventListener('click',()=>runBusy(async()=>{const info=actions?.inspectPublicOpinion?.();if(!info?.simulationReady){notify('请先完成至少一次世界推演，建立当前世界状态后再生成见闻','warning');return}if(!confirm('“街巷漫游”将调用一次模型，同时生成生活化市井闲闻和 3～5 个地点建议，不会写入世界事实。继续吗？'))return;try{const result=await actions?.refreshStreetOpinion?.();{const ref=result?.worldReference;const refText=ref?` · 参考 ${ref.selectedEntries||0} 条世界书条目`:'';notify(`街巷漫游已更新：${result?.itemCount||0} 条市井闲闻，${result?.placeCount||0} 个地点${refText}`,'success')}}catch(error){console.error('[SceneWorld] street opinion failed',error);notify(`街巷漫游失败：${error?.message||error}`,'error')}}));
        ...
        shadow.querySelectorAll('[data-remove-person]').forEach(button=>button.addEventListener('click',event=>{event.preventDefault();event.stopPropagation();const state=readSceneWorldState();const person=(state?.people||[]).find(item=>item?.id===button.dataset.removePerson);if(!person)return;if(!confirm(`确定删除人物“${person.name}”吗？后续正文再次明确出现时，世界推演仍可能重新识别该人物。`))return;runBusy(async()=>{try{await actions?.removePerson?.(person.id);notify('人物已删除','success')}catch(error){console.error('[SceneWorld] person remove failed',error);notify(`删除人物失败：${error?.message||error}`,'error')}})}));
        ...
        shadow.querySelectorAll('[data-clear-section]').forEach(button=>button.addEventListener('click',()=>{const labels={people:'人物',observation:'见闻',continuity:'脉络',chronicle:'纪事',guidance:'行动建议'};const section=button.dataset.clearSection;const label=labels[section]||section;if(!confirm(`只清理当前聊天的“${label}”数据，其他世界动态内容保留。确定继续吗？`))return;runBusy(async()=>{try{const removed=await actions?.clearSection?.(section);notify(removed?`${label}数据已清理`:'当前聊天尚未建立世界动态数据','success')}catch(error){console.error('[SceneWorld] section clear failed',error);notify(`清理${label}失败：${error?.message||error}`,'error')}})}));
