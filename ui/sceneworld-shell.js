...
function actionSuggestionsHtml(state, busy) {
    ...
    if (!state) return '';
    return `<div class="card"><div class="section-title"><h3>行动建议</h3><span class="count">${actions.length} 条</span></div><p>每次世界推演会顺手生成 3～5 条下一步方向，不额外调用 API；这些只是灵感，不是世界事实。</p>${actions.length?`<div class="item-list">${actions.map(item=>`<div class="item"><div class="item-head"><b>${escapeHtml(item.title)}</b><button class="insert-small" type="button" data-insert-guidance="${escapeHtml(item.id)}" data-guidance-kind="action" ${busy?'disabled':''}>填入输入框</button></div>${item.tone?`<div class="chips"><span class="chip">${escapeHtml(item.tone)}</span></div>`:''}${item.reason?`<p>${escapeHtml(item.reason)}</p>`:''}<div class="quote">${escapeHtml(item.prompt)}</div></div>`).join('')}</div>`:'<p class="empty">完成一次世界推演后，这里会随本轮结果生成行动建议。</p>'}</div>`;
}
...
function homeHtml(version, preview, busy) {
    ...
    return `<div class="stack">
        <div class="card"><h2>2.0 重构 · 阶段 4B：世界信息与下一步灵感</h2><p>核心推演继续只结算 AI 正文；世界信息、随便逛逛和下一步灵感都是独立手动任务，不会自动写入正文。</p><div class="status-grid"><div class="status"><b>插件</b><span>已就绪 · ${escapeHtml(version)}</span></div><div class="status"><b>sceneworld 数据</b><span>${info.dataExists?`已创建 · ${formatBytes(info.totalBytes)}`:'未创建'}</span></div><div class="status"><b>正文同步</b><span>${escapeHtml(latestSyncText(state))}</span></div><div class="status"><b>自动推演</b><span>未加载 · 当前仅手动</span></div></div></div>
...
function renderTab(tab,version,preview,busy){const state=readSceneWorldState();if(tab==='此刻')return homeHtml(version,preview,busy);if(tab==='人物')return peopleHtml(state);if(tab==='见闻')return opinionHtml(state,busy);if(tab==='记忆')return memoryHtml(state);if(tab==='纪事')return chronicleHtml(state,busy);return emptySection(tab)}
