...
function baiBaiStatusText(status) { return status?.available ? `已检测到记忆插件接口${status.pluginVersion?` · ${status.pluginVersion}`:''}` : '当前未检测到记忆插件接口；不可用时会自动跳过'; }
...
function worldReferenceSettingsHtml(actions, busy, entryChoices) {
    ...
    return `<div class="card"><div class="card-header"><div><h2>世界观与长期参考</h2><div class="card-subtitle">世界推演和见闻分别选择要传输的条目</div></div>${helpButton('reference','查看参考资料规则')}</div>
      ...
      <div class="setting-grid">
        ...
        <label class="setting-row"><input type="checkbox" data-world-ref-setting="simulationUseBaiBaiBook" ${settings.simulationUseBaiBaiBook===true?'checked':''} ${busy?'disabled':''}><span><b>世界推演使用记忆插件长期历史</b><small><span data-baibai-status="simulation">${escapeHtml(baiBaiStatusText(simulationBaiBai))}</span>。默认最多 ${Number(settings.baibaiHistoryMaxChars)||8000} 字符。</small></span></label>
      </div>
...
function modelAdvancedSettingsHtml(actions, busy) {
    ...
    return `<details class="card api-advanced model-advanced-card"><summary>高级设置（通常无需修改）</summary><div class="api-model-body">
        ...
        <div class="note">读取待推演剧情后会自动估算完整输入。使用酒馆当前连接且能够读取上下文上限时，会提前做安全检查；自定义 API 无法可靠识别上限时只显示输入估算，不要求填写 32K、128K 等技术参数。</div>
    </div></details>`;
}
