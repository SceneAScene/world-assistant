...
.star{flex:0 0 auto;border:1px solid rgba(255,255,255,.12);background:transparent;color:inherit;border-radius:8px;min-width:34px;height:30px;cursor:pointer;opacity:.78}.star[disabled]{opacity:.35;cursor:default}.remove-small{border:0;background:transparent;color:inherit;opacity:.6;cursor:pointer;font-size:12px;padding:2px 4px}.insert-small{border:1px solid rgba(255,255,255,.14);background:rgba(255,255,255,.06);color:inherit;border-radius:8px;padding:6px 9px;cursor:pointer;font-size:11px;white-space:nowrap}.insert-small.icon-only{width:34px;height:30px;padding:0;display:grid;place-items:center;font-size:18px;line-height:1;font-weight:700}.insert-small:disabled{opacity:.4;cursor:not-allowed}.quote{margin-top:8px;padding:9px 10px;border-left:3px solid rgba(180,170,255,.35);background:rgba(255,255,255,.025);font-size:12px;line-height:1.65}.street-meta{font-size:11px;opacity:.62;margin-top:4px}.section-title{display:flex;align-items:center;gap:8px;margin:14px 0 7px}.section-title h3{margin:0;flex:1}.section-title .count{font-size:11px;opacity:.5}.setting-grid{display:grid;gap:8px;margin-top:10px}.setting-row{display:flex;align-items:flex-start;gap:9px;padding:9px 10px;border-radius:9px;background:rgba(0,0,0,.12)}.setting-row input{margin-top:2px;accent-color:currentColor}.setting-row span{font-size:12px;line-height:1.5}.setting-row small{display:block;opacity:.58;margin-top:2px}.setting-text{width:100%;margin-top:8px;border:1px solid rgba(255,255,255,.14);background:rgba(0,0,0,.16);color:inherit;border-radius:9px;padding:9px 10px;font:inherit;font-size:12px;outline:none}.setting-text:focus{border-color:rgba(157,147,240,.55)}.book-picker{margin-top:10px;border:1px solid rgba(255,255,255,.11);border-radius:10px;background:rgba(0,0,0,.10);overflow:hidden}.book-picker summary{cursor:pointer;padding:10px 12px;font-size:12px;font-weight:650;list-style-position:inside}.book-list{max-height:220px;overflow:auto;padding:0 8px 8px;display:grid;gap:6px}.book-row{display:flex;align-items:flex-start;gap:9px;padding:8px 9px;border-radius:8px;background:rgba(255,255,255,.035)}.book-row input{margin-top:2px;accent-color:currentColor}.book-row span{min-width:0;font-size:12px;line-height:1.45}.book-row small{display:block;opacity:.55;margin-top:2px;word-break:break-word}
...
function actionSuggestionsHtml(state, busy) {
    ...
    if (!state) return '';
    return `<div class="card"><div class="section-title"><h3>行动建议</h3><span class="count">${actions.length} 条</span></div><p>每次世界推演会顺手生成 3～5 条下一步方向，不额外调用 API；这些只是灵感，不是世界事实。</p>${actions.length?`<div class="item-list">${actions.map(item=>`<div class="item"><div class="item-head"><b>${escapeHtml(item.title)}</b><button class="insert-small icon-only" type="button" title="填入输入框" aria-label="填入输入框" data-insert-guidance="${escapeHtml(item.id)}" data-guidance-kind="action" ${busy?'disabled':''}>↪</button></div>${item.tone?`<div class="chips"><span class="chip">${escapeHtml(item.tone)}</span></div>`:''}${item.reason?`<p>${escapeHtml(item.reason)}</p>`:''}<div class="quote">${escapeHtml(item.prompt)}</div></div>`).join('')}</div>`:'<p class="empty">完成一次世界推演后，这里会随本轮结果生成行动建议。</p>'}</div>`;
}
...
function opinionHtml(state, busy) {
    ...
    return `<div class="stack">
      ...
      <div class="card"><div class="section-title"><h3>去哪逛逛</h3><span class="count">${places.length} 条</span></div><p>每次街巷漫游同时生成 3～5 个可探索地点；“灵感地点”只是建议，不会自动变成正史地点。</p>${places.length?`<div class="item-list">${places.map(item=>`<div class="item"><div class="item-head"><b>${escapeHtml(item.name)}</b><button class="insert-small icon-only" type="button" title="填入输入框" aria-label="填入输入框" data-insert-guidance="${escapeHtml(item.id)}" data-guidance-kind="place" ${busy?'disabled':''}>↪</button></div><div class="chips">${item.type?`<span class="chip">${escapeHtml(item.type)}</span>`:''}<span class="chip">${item.established?'已有地点':'灵感地点'}</span></div>${item.why?`<p>${escapeHtml(item.why)}</p>`:''}<div class="quote">${escapeHtml(item.prompt)}</div></div>`).join('')}</div>`:'<p class="empty">还没有生成地点推荐。</p>'}</div>
    </div>`;
}
...
function continuityHtml(state, busy, actions) {
    ...
    const sourceLabel = item => {
        const start = Number.isInteger(item?.sourceStartMessageId) ? item.sourceStartMessageId : null;
        const end = Number.isInteger(item?.sourceEndMessageId) ? item.sourceEndMessageId : (Number.isInteger(item?.sourceMessageId) ? item.sourceMessageId : null);
        if (start===null && end===null) return '';
        if (start!==null && end!==null && start!==end) return `AI #${start}～#${end}`;
        return `AI #${end??start}`;
    };
    ...
    const baibaiText = !baibai.enabled ? '未启用' : baibai.available ? '已启用 · 推演时按预算读取压缩历史' : '已启用 · 当前接口未检测到，推演时自动跳过';
    return `<div class="stack">
      <div class="card"><h2>脉络</h2><p>这里不是完整剧情记忆库。世界动态只保留“现在仍然有效的世界连续性”以及最近 5 次世界推演摘要，较久以前的剧情可按需交给柏宝书负责。</p><div class="status-grid"><div class="status"><b>最近世界动态</b><span>${recent.length} / 5 次</span></div><div class="status"><b>持续性世界事实</b><span>${facts.length} / 20 条</span></div><div class="status"><b>长期历史</b><span>${escapeHtml(baibaiText)}</span></div><div class="status"><b>数据策略</b><span>覆盖当前状态 · 滚动保留最近记录</span></div></div></div>
      <div class="card"><div class="section-title"><h3>最近 5 次世界动态</h3><span class="count">${recent.length} 条</span></div><p>每次世界推演只保存一条简短净变化摘要；超过 5 条自动淘汰最旧记录。</p>${recent.length?`<div class="item-list">${recent.map((item,index)=>`<div class="item"><b>${escapeHtml(item.summary)}</b><div class="meta">${escapeHtml(sourceLabel(item) || `最近第 ${index+1} 条`)}${item.createdAt?` · ${escapeHtml(timeLabel(item.createdAt))}`:''}</div></div>`).join('')}</div>`:'<p class="empty">当前还没有最近世界动态。</p>'}</div>
      <div class="card"><div class="section-title"><h3>持续性世界事实</h3><span class="count">${facts.length} / 20</span></div><p>只保留当前或长期仍会约束后续世界一致性的事实。AI 识别错误时可以手动修改或删除；修改后的内容会继续参与后续世界推演。</p>${sortedFacts.length?`<div class="item-list">${sortedFacts.map(item=>`<div class="item"><div class="item-head"><b>${escapeHtml(item.value||item.key||'持续性世界事实')}</b><div class="mini-actions"><button class="remove-small" type="button" title="修改这条事实" data-edit-continuity-fact="${escapeHtml(item.id)}" ${busy?'disabled':''}>修改</button><button class="remove-small" type="button" title="删除这条事实" data-remove-continuity-fact="${escapeHtml(item.id)}" ${busy?'disabled':''}>删除</button></div></div><div class="chips">${item.key?`<span class="chip">${escapeHtml(item.key)}</span>`:''}<span class="chip">${escapeHtml(validityLabel(item.validity))}</span><span class="chip">${escapeHtml(publicityLabel(item.publicity))}</span>${item.source==='manual'?'<span class="chip">手动修正</span>':''}</div>${item.publicHint?`<p>公开表象：${escapeHtml(item.publicHint)}</p>`:''}${item.evidence?`<details class="preview-details"><summary>查看依据</summary><div class="note">${escapeHtml(item.evidence)}</div></details>`:''}<div class="meta">${escapeHtml([sourceLabel(item),item.updatedAt?`更新：${timeLabel(item.updatedAt)}`:''].filter(Boolean).join(' · '))}</div></div>`).join('')}</div>`:'<p class="empty">当前没有需要长期占用额度的持续性世界事实。</p>'}</div>
    </div>`;
}
function chronicleHtml(state, busy) {
    ...
    if(!items.length)return `<div class="stack"><div class="card"><h2>纪事</h2><p class="empty">暂无收藏。新闻、论坛和“市井闲闻”里的 ☆ 可以把喜欢的内容保留下来。</p></div></div>`;
