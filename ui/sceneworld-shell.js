...
function worldReferenceSettingsHtml(actions, busy) {
    ...
    const row = (key, title, hint) => `<label class="setting-row"><input type="checkbox" data-world-ref-setting="${key}" ${settings[key] !== false ? 'checked' : ''} ${busy?'disabled':''}><span><b>${title}</b><small>${hint}</small></span></label>`;
    const globalChecked = settings.includeGlobalWorldInfo === true;
    return `<div class="card"><h2>世界观参考</h2><p>只在真正调用模型前读取，用于约束人物基础、时代、地理和世界规则；不会把世界书背景直接当成本轮已发生事件，也不会因为打开界面就写入聊天数据。</p><div class="setting-grid">${row('includeCharacterBase','角色卡基础设定','默认开启：背景、性格、场景等基础资料。')}${row('includeCharacterWorldInfo','角色绑定世界书','默认开启：按本轮正文与现有状态的关键词只选相关条目。')}${row('includeChatWorldInfo','当前聊天世界书','默认开启：只读取当前聊天绑定的 Lorebook。')}<label class="setting-row"><input type="checkbox" data-world-ref-setting="includeGlobalWorldInfo" ${globalChecked?'checked':''} ${busy?'disabled':''}><span><b>全局世界书</b><small>默认关闭：全局书往往较多、较杂；需要时再手动开启。</small></span></label></div><div class="note">世界书条目采用本地关键词匹配和字符预算筛选，不额外调用 AI 来判断“该读哪条”。</div></div>`;
}
...
function homeHtml(version, preview, busy, actions) {
    ...
    return `<div class="stack">
        <div class="card"><h2>2.0 重构 · 阶段 4C：世界观参考与显示规范</h2><p>核心推演继续只结算 AI 正文；世界信息、街巷漫游和下一步灵感都是独立手动任务，不会自动写入正文。</p><div class="status-grid"><div class="status"><b>插件</b><span>已就绪 · ${escapeHtml(version)}</span></div><div class="status"><b>sceneworld 数据</b><span>${info.dataExists?`已创建 · ${formatBytes(info.totalBytes)}`:'未创建'}</span></div><div class="status"><b>正文同步</b><span>${escapeHtml(latestSyncText(state))}</span></div><div class="status"><b>自动推演</b><span>未加载 · 当前仅手动</span></div></div></div>
...
function renderTab(tab,version,preview,busy,actions){const state=readSceneWorldState();if(tab==='此刻')return homeHtml(version,preview,busy,actions);if(tab==='人物')return peopleHtml(state);if(tab==='见闻')return opinionHtml(state,busy);if(tab==='记忆')return memoryHtml(state);if(tab==='纪事')return chronicleHtml(state,busy);return emptySection(tab)}
...
export function createSceneWorldShell({ version, onClose, actions }) {
    ...
    const render=()=>{
        ...
        shadow.querySelector('[data-action="simulate"]')?.addEventListener('click',()=>runBusy(async()=>{const batch=preview?.batch;if(!batch?.canSimulate)return;if(!confirm(`将调用一次当前 SillyTavern 模型，结算 #${batch.startId}～#${batch.endId} 共 ${batch.assistantCount} 条 AI 正文。继续吗？`))return;try{const result=await actions?.simulatePending?.(batch);preview=actions?.inspectPendingNarrative?.()??preview;const count=countReportedChanges(result?.changeSummary);const ref=result?.worldReference;const refText=ref?` · 世界观参考：${ref.characterBaseUsed?'角色卡基础 + ':''}${ref.matchedEntries||0} 条世界书`:'';notify((count?`世界推演完成：结算 #${result.batch.startId}～#${result.batch.endId}，保存 ${count} 组变化`:`世界推演完成：结算 #${result.batch.startId}～#${result.batch.endId}，本轮没有值得额外记录的变化`)+refText,'success')}catch(error){console.error('[SceneWorld] pending simulation failed',error);notify(`世界推演失败：${error?.message||error}`,'error')}}));
        shadow.querySelector('[data-action="refresh-opinion"]')?.addEventListener('click',()=>runBusy(async()=>{try{const info=actions?.inspectPublicOpinion?.();if(info?.sourceCount>0&&!confirm(`将根据当前 ${info.publicCount} 条 public、${info.traceCount} 条 trace 世界事实调用一次模型刷新新闻与论坛。继续吗？`))return;const result=await actions?.refreshPublicOpinion?.();if(!result?.calledModel)notify('当前没有可用于公共动态的公开世界变化，新闻与论坛已保持为空','info');else if((result.newsCount||0)+(result.forumCount||0)===0)notify('公共动态刷新完成：当前没有值得记录的新闻或公共讨论','success');else {const ref=result?.worldReference;const refText=ref?` · 参考 ${ref.matchedEntries||0} 条世界书`:'';notify(`公共动态刷新完成：${result.newsCount||0} 条新闻，${result.forumCount||0} 条讨论${refText}`,'success')}}catch(error){console.error('[SceneWorld] public info failed',error);notify(`刷新公共动态失败：${error?.message||error}`,'error')}}));
        shadow.querySelector('[data-action="refresh-street"]')?.addEventListener('click',()=>runBusy(async()=>{if(!confirm('“街巷漫游”将调用一次模型，同时生成 NON-CANON 市井闲闻和 3～5 个地点建议，不会写入世界事实。继续吗？'))return;try{const result=await actions?.refreshStreetOpinion?.();{const ref=result?.worldReference;const refText=ref?` · 参考 ${ref.matchedEntries||0} 条世界书`:'';notify(`街巷漫游已更新：${result?.itemCount||0} 条市井闲闻，${result?.placeCount||0} 个地点${refText}`,'success')}}catch(error){console.error('[SceneWorld] street opinion failed',error);notify(`街巷漫游失败：${error?.message||error}`,'error')}}));
