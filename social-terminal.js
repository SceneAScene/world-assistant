...
export function buildMomentsPrompt(social, state, { userName = '你' } = {}) {
    ...
    return [
        '你正在生成“世界动态·朋友圈”的自然动态候选。这是人物主动发布的社交内容，不是旁白，也不是正式世界事实。',
        '系统仅收取人物公开发布的生活片段，不会因此获得人物未公开的信息。',
...
export function buildSocialPulsePrompt(social, state, { userName = '你' } = {}) {
    ...
    return [
        ...
        '系统仅负责消息传递，不具备全知信息；传递过程不能替人物补足未知信息。',
...
export function buildSocialReplyPrompt(social, state, conversationId, { userName = '你' } = {}) {
    ...
    return [
        '你正在执行“世界动态·内置社交”回复路由。这是一个独立社交记录层，不是正文，也不会自动成为正式世界事实。',
        '系统只负责传递双方明确发送的信息，不替人物回答，也不会向人物注入其不应知晓的后台知识。',
        '必须严格执行：',
